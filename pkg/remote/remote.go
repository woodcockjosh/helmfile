...
func S3FileExists(path string) error {
  ...
  // Create a new AWS session using the default AWS configuration and region
  sess := session.Must(session.NewSessionWithOptions(session.Options{
    SharedConfigState: session.SharedConfigEnable,
  }))
  if err != nil {
  ...
  // Perform a HEAD request on the S3 object
