...
func TestRemote_S3(t *testing.T) {
  ...
  cachefs := map[string]string{
    filepath.Join(CacheDir(), "s3_helm-s3-values-example/values.yaml"): "foo: bar",
  }
  ...
  for i := range testcases {
    ...
    t.Run(fmt.Sprintf("case %d", i), func(t *testing.T) {
      ...
      get := func(wd, src, dst string) error {
        ...
        if src != "s3://helm-s3-values-example/values.yaml" {
          return fmt.Errorf("unexpected src: %s", src)
        }
      ...
      url := "s3://helm-s3-values-example/values.yaml"
      file, err := remote.Locate(url)
      if err != nil {
        t.Fatalf("unexpected error: %v", err)
      }

      expectedFile := filepath.Join(CacheDir(), "s3_helm-s3-values-example/values.yaml")
      if file != expectedFile {
        t.Errorf("unexpected file located: %s vs expected: %s", file, expectedFile)
      }
